...
class LayerManager:
    ...
    @classmethod
    def update_layer(cls, layer_id, content, layer_type):
        """Update a layer with new content."""
        if layer_id not in cls._layers:
            return {'error': 'Layer not found'}
        
        layer = cls._layers[layer_id]
        ...
        if layer_type == 'text':
            ...
            cls._adjust_text_layer(layer)
        elif layer_type == 'image':
            layer['content'] = content
            # Smart crop and resize image
            cls._adjust_image_layer(layer)
    ...
    @classmethod
    def export_document(cls, size, format='png'):
        """Export the document with the specified size."""
        if not cls._document:
